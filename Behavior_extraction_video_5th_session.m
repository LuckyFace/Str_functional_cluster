...
xFile = FindFiles('Raw*.xlsx','CheckSubdirs',0);
xlsFile = xlsread(xFile{1});

% in case of D1_7 refine to 8min_

% ratio correction - x and y axis
ratx = (max((Deeptrace{session,1}(:,2)))-min((Deeptrace{session,1}(:,2))))/(max(xlsFile(:,3))-min(xlsFile(:,3)));
raty = (max((Deeptrace{session,1}(:,3)))-min((Deeptrace{session,1}(:,3))))/(max(xlsFile(:,4))-min(xlsFile(:,4)));
Dtrace = Deeptrace{session,1};
Dtrace(:,[2 5 8]) = Dtrace(:,[2 5 8])/ratx;
Dtrace(:,[3 6 9]) = Dtrace(:,[3 6 9])/raty;
% calculate distance and velocity
for i=1:length(Dtrace)
    N(i,1) = Dtrace(i,2) + 1i*Dtrace(i,3);    
    C_etho(i,1) = xlsFile(i,3) + 1i*xlsFile(i,4);
    C(i,1) = Dtrace(i,5) + 1i*Dtrace(i,6);
    T(i,1) = Dtrace(i,8) + 1i*Dtrace(i,9);
    F(i,1) = Dtrace(i,1);
end  
...
% % finding the real start
% % once thloc start detected --> find local minimum before 100 frame, loast
% for i=1:length(thlocstr)
%     if thlocstr2(i)>0
%     tempC = Cv10(thlocstr2(i):thlocstr(i));
%     locmin = find(tempC == min(tempC),1,'first');
%     thlocstr3(i,1) = thlocstr2(i) + locmin;
%     locmin=[];tempC=[];
%     end
% end
% complete stop
% Cv and Nv both below mean;
 Nv10 = imgaussfilt(Nv,10);
...
% time = (F+1)*0.033333333333333333333333333333333333;
time = xlsFile(:,1);
time2 = (F+1)*(1/30);
